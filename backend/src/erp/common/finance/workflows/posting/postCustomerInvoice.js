...
async function postCustomerInvoice(req, invoiceId) {
  ...
  invoice.ledgerPosting = { postingState: "posted", postingKey: `customer_invoice:${invoice._id}`, postedAt: new Date() };
  invoice.statusHistory.push({ status: "posted", changedBy: asText(req.user.uid), note: "Customer receivable posted" });
  await invoice.save();
  return invoice;
}
