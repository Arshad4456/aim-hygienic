...
function getAllowedOrigins() {
  ...
  if (appDomain && appDomain !== "yourdomain.com") { defaults.push(`https://${appDomain}`); defaults.push(`https://www.${appDomain.replace(/^www\./, "")}`); }
  return [...new Set([...defaults, ...configured])];
}
function buildCorsOptions() { const allowedOrigins = getAllowedOrigins(); return { origin(origin, callback) { if (!origin || allowedOrigins.includes(origin)) return callback(null, true); return callback(new Error("Not allowed by CORS")); }, credentials: true }; }
