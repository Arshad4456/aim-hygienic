...
const { ensureDocumentPresets, ensurePlatformTemplates } = require("../../services/platformTemplateBootstrapService");
...
router.get('/document-template-presets', async (_req, res) => { await ensureDocumentPresets(); return res.json({ success: true, presets: await DocumentTemplatePreset.find({ isActive: true }).sort({ documentType: 1, templateName: 1 }).lean() }); });
...
router.get('/setup-templates', async (_req, res) => { await ensurePlatformTemplates(); return res.json({ success: true, templates: await CompanySetupTemplate.find({ isActive: true }).sort({ createdAt: -1 }).lean() }); });
