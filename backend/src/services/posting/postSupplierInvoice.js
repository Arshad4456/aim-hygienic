...
async function postSupplierInvoice(req, invoiceId) {
  ...
  invoice.ledgerPosting = { postingState: "posted", postingKey: `supplier_invoice:${invoice._id}`, postedAt: new Date() };
  invoice.statusHistory.push({ status: "posted", changedBy: asText(req.user.uid), note: "Supplier payable posted" });
  await invoice.save();
  return invoice;
}
