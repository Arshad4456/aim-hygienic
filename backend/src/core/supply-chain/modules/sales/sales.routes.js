...
function salesModuleFor(req) {
  const path = String(req.path || "");
  if (path.includes("secondary")) return "secondary-sales-orders";
  if (path.includes("quotation")) return "sales-quotations";
  if (path.includes("customer")) return "customer-billing";
  if (path.includes("receipt")) return "receipts";
  if (path.includes("dispatch") || path.includes("pod")) return "dispatches";
  return "primary-sales-orders";
}
router.use(requireAuth, requireCompanyModule(salesModuleFor));
