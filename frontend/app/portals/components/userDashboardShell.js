...
export default function UserDashboardShell({ title, subtitle, roleKey, links = [], showAccountCards = false, children = null }) {
  ...
  const settingsHref = roleSlug ? `/portals/${roleSlug}/settings` : "/portals/admin/settings";
  const changePasswordHref = roleSlug ? `/portals/${roleSlug}/settings/change-password` : "/portals/admin/settings/change-password";
