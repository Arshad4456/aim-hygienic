...
export default function FieldListPage() {
  ...
  const load = async () => {
    ...
    setErr("");
    try {
      const data = await listFieldsCompat();
      setRows(data.fields || []);
    } catch (e) {
  ...
  async function save() {
    try {
      await updateFieldCompat(edit._id, edit);
      setEdit(null);
      load();
    } catch (e) {
          ...
          <table className="min-w-[1100px] w-full text-sm">
            <thead className="bg-zinc-50">
              <tr>
                ...
                <th className="px-3 py-2 border-b text-left">Warehouse</th>
                <th className="px-3 py-2 border-b text-left">Region</th>
                <th className="px-3 py-2 border-b text-left">Zone</th>
                <th className="px-3 py-2 border-b text-left">Territory</th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="px-3 py-6 text-center">Loading...</td></tr>
              ) : (
                rows.map((r) => (
                  <tr key={r._id}>
                    <td className="px-3 py-2 border-b">{r.fieldId}</td>
                    <td className="px-3 py-2 border-b">{r.name}</td>
                    <td className="px-3 py-2 border-b">{r.warehouseName || "-"}</td>
                    <td className="px-3 py-2 border-b">{r.regionName || "-"}</td>
                    <td className="px-3 py-2 border-b">{r.zoneName || "-"}</td>
                    <td className="px-3 py-2 border-b">{r.territoryName || "-"}</td>
                    <td className="px-3 py-2 border-b">{r.status || "active"}</td>
                    <td className="px-3 py-2 border-b">
                      <button className="rounded-lg border px-3 py-1 text-xs mr-2" onClick={() => setEdit({ ...r })}>Edit</button>
                      <button className="rounded-lg border px-3 py-1 text-xs text-red-600" onClick={() => del(r._id)}>Delete</button>
                    </td>
                  </tr>
                ))
              )}
