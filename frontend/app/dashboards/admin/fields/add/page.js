...
export default function AddFieldPage() {
  ...
  const wh = warehouses.find((x) => x._id === form.warehouseDocId);
  const rg = regions.find((x) => x._id === form.regionDocId);
  const zn = zones.find((x) => x._id === form.zoneDocId);

  const regionOpts = useMemo(
    () => regions.filter((r) => !wh || r.warehouseId === wh.warehouseId),
    [regions, wh],
  );
  const zoneOpts = useMemo(
    () => zones.filter((z) => (!wh || z.warehouseId === wh.warehouseId) && (!rg || z.regionId === rg.regionId)),
    [zones, wh, rg],
  );
  ...
  async function submit(e) {
    ...
    setOk("");
    try {
      const tr = territories.find((t) => t._id === form.territoryDocId);
    ...
    <AdminShell title="Add Field" user={null}>
      <div className="rounded-2xl bg-white border shadow-sm p-5">
        ...
        <form onSubmit={submit} className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Select label="Warehouse Name" value={form.warehouseDocId} onChange={(v) => setForm((s) => ({ ...s, warehouseDocId: v, regionDocId: "", zoneDocId: "", territoryDocId: "" }))} options={warehouses.map((w) => ({ value: w._id, label: w.name }))} required />
