    ...
    try {
      await apiFetch("/companies", {
        ...
        body: {
          ...
          branches: parseList(form.branchesText),
          warehouses: parseList(form.warehousesText),
          products: parseList(form.productsText),
        },
      });
    ...
    <AdminShell title="Add New Company" user={null}>
      <div className="rounded-2xl bg-white border shadow-sm p-5">
        ...
        <div className="text-sm text-zinc-500 mt-1">
          Enter company details. Branches/Warehouses/Products are comma-separated.
        </div>
        ...
        <form onSubmit={onSubmit} className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Company Code" value={form.code} onChange={(v) => setField("code", v)} required />
          <Field label="Company Name" value={form.name} onChange={(v) => setField("name", v)} required />
          <Field label="Company Manager Name" value={form.managerName} onChange={(v) => setField("managerName", v)} />
          ...
          <div className="md:col-span-2">
            <Label>Company Branches (comma separated)</Label>
            <input
              className="mt-1 w-full rounded-xl border px-3 py-2 outline-none focus:ring-2 focus:ring-emerald-200"
              value={form.branchesText}
              onChange={(e) => setField("branchesText", e.target.value)}
              placeholder="e.g. Rawalpindi, Islamabad, Lahore"
            />
          </div>

          <div className="md:col-span-2">
            <Label>Company Warehouses (comma separated)</Label>
            <input
              className="mt-1 w-full rounded-xl border px-3 py-2 outline-none focus:ring-2 focus:ring-emerald-200"
              value={form.warehousesText}
              onChange={(e) => setField("warehousesText", e.target.value)}
              placeholder="e.g. RWP Main, ISB Central"
            />
          </div>

          <div className="md:col-span-2">
            <Label>Company Products (comma separated)</Label>
            <input
              className="mt-1 w-full rounded-xl border px-3 py-2 outline-none focus:ring-2 focus:ring-emerald-200"
              value={form.productsText}
              onChange={(e) => setField("productsText", e.target.value)}
              placeholder="e.g. Mamia Diaper, Mamia Wipes, Royal Baby"
            />
          </div>
