...
import { salesDashboardSearchItems } from "../../searchItems";
...
export default function SalesShell({ children, user, title = "Sales Dashboard" }) {
  ...
  const filteredSearchItems = useMemo(() => {
    ...
    if (!value) return salesDashboardSearchItems;
    return salesDashboardSearchItems.filter((item) => item.title.toLowerCase().includes(value));
  }, [searchTerm]);
