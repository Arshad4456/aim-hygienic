...
export default function SalesDashboardPage() {
  ...
  return (
    <AdminShell user={user} title="Sales Manager Dashboard">
        ...
        <div className="rounded-2xl bg-white border shadow-sm p-5 flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-semibold text-zinc-900">
              {user?.companyName || user?.company || "AIM Hygienic (Pvt) Limited"}
            </div>
            <div className="text-sm text-zinc-500 mt-1">
              Sales manager: <span className="font-medium text-zinc-800">{user?.fullName || "Sales Lead"}</span>
            </div>
            <div className="text-xs text-zinc-400 mt-1">
              Company scoped performance snapshot.
            </div>
          </div>
