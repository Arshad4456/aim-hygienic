...
export default function PortalRouteLoader({ slug = [] }) {
  ...
  useEffect(() => {
    if (!loading && workingPath && pathname !== workingPath && route.moduleKey !== "dashboard") {
      router.replace(workingPath);
    }
  }, [loading, pathname, route.moduleKey, router, workingPath]);

  const subtitle = route.isLegacyAlias
    ? `Legacy portal path mapped to ${route.canonicalPath}. This module now belongs to Rawyan ERP permissions.`
    : route.module?.description;

  return (
    <PortalShell title={route.title} subtitle={subtitle} user={user} menu={menu}>
      {loading ? (
        <div className="rounded-3xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-sm">
          Loading your portal permissions…
        </div>
      ) : null}
      {error ? (
        <div className="mb-4 rounded-3xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700">
          Could not load live permissions. Showing the safe default portal menu.
        </div>
      ) : null}
      {!loading && workingPath && pathname !== workingPath && route.moduleKey !== "dashboard" ? (
        <div className="rounded-3xl border border-cyan-200 bg-cyan-50 p-5 text-sm text-cyan-800">
          Opening the working Rawyan ERP module…
        </div>
      ) : (
