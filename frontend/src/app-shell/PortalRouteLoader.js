...
export default function PortalRouteLoader({ slug = [] }) {
  ...
  useEffect(() => { if (!loading && workingPath && pathname !== workingPath && !["dashboard", "roles", "users", "territory", "procurement", "purchase-orders", "supplier-payments", "goods-receipts"].includes(route.moduleKey)) router.replace(workingPath); }, [loading, pathname, route.moduleKey, router, workingPath]);
  ...
  const shouldRedirect = !loading && workingPath && pathname !== workingPath && !["dashboard", "roles", "users", "territory", "procurement", "purchase-orders", "supplier-payments", "goods-receipts"].includes(route.moduleKey);
