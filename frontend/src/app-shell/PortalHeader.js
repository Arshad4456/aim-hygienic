      ...
      <div className="mx-auto flex max-w-[1600px] flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">
            <span className="rounded-full bg-slate-100 px-3 py-1">{user?.companyName || "Company Portal"}</span>
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-emerald-700">{user?.role || user?.portalType || "ERP User"}</span>
          </div>
        </div>
