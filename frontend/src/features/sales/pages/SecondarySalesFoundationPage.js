...
import secondarySalesService from "../../../services/secondarySalesService";
...
const emptyLine = { productId: "", productCode: "", productName: "", warehouseId: "", availableQty: 0, qty: 1, unitPrice: 0, unitCost: 0 };
function money(value) { return `PKR ${Number(value || 0).toLocaleString()}`; }
function nameOf(row = {}) { return row.partyName || row.customerName || row.fullName || row.username || row.productName || row.name || "-"; }
...
export default function SecondarySalesFoundationPage() {
  ...
  const [form, setForm] = useState({ customerId: "", notes: "", lines: [{ ...emptyLine }] });

  async function load() {
    setLoading(true); setError("");
    try {
      const [overviewRes, customersRes, productsRes, ordersRes, invoicesRes, receiptsRes] = await Promise.all([
        secondarySalesService.overview(), secondarySalesService.customers(), secondarySalesService.products(), secondarySalesService.orders(), secondarySalesService.invoices(), secondarySalesService.receipts(),
      ]);
  ...
  function setLine(index, key, value) {
    setForm((current) => {
      ...
      if (key === "productId") {
        const product = products.find((p) => String(`${p.productId || ""}:${p.warehouseId || ""}`) === String(value));
  ...
  async function saveOrder() {
    ...
    const lines = form.lines.filter((line) => line.productName && Number(line.qty || 0) > 0).map((line) => ({ productId: line.productId, productCode: line.productCode, productName: line.productName, qty: Number(line.qty || 0), unitPrice: Number(line.unitPrice || 0), unitCost: Number(line.unitCost || 0), warehouseId: line.warehouseId }));
    if (!customer) { setError("Select customer first."); return; }
    if (!lines.length) { setError("Add at least one distributor stock item."); return; }
    await action("Secondary sales order", () => secondarySalesService.createOrder({ customerId: form.customerId, customer: { partyId: customer._id || customer.userId || customer.customerId, partyName: nameOf(customer), mobile: customer.mobile || customer.mobileNumber, address: customer.address || customer.shopAddress }, lines, totals: { grandTotal: total }, notes: form.notes }));
    setForm({ customerId: "", notes: "", lines: [{ ...emptyLine }] });
    setTab("orders");
  }

  return <div className="space-y-6">
    <div className="rounded-[2rem] bg-gradient-to-r from-emerald-50 via-cyan-50 to-blue-50 p-6 ring-1 ring-cyan-100"><p className="text-xs font-black uppercase tracking-[0.35em] text-emerald-600">Phase 7 Secondary Sales</p><h2 className="mt-2 text-3xl font-black text-slate-950">Distributor → Customer</h2><p className="mt-2 max-w-4xl text-sm text-slate-600">Create customer orders from distributor stock, approve them, deliver/invoice, and receive customer payments.</p></div>
    <div className="rounded-3xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><b>Correct flow:</b> company stock must first move to distributor stock through Primary Sales. Secondary sales then reduce distributor stock and create customer invoice.</div>
