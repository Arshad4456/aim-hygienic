...
const emptyForm = { fullName: "", username: "", mobile: "", email: "", password: "", roleId: "", companyId: "", companyName: "", mobileAccess: false };
...
export default function UsersAccessPage({ companyOptions = [], systemMode = false }) {
  ...
  async function createUser() {
    ...
    try {
      const selectedCompany = companyOptions.find((c) => String(c.companyId) === String(form.companyId));
      const payload = { ...form, companyName: form.companyName || selectedCompany?.name || "" };
  ...
  return <div className="space-y-5">
    ...
    {showCreate ? <div className="rounded-3xl border border-emerald-100 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500">Select company only when System Admin is creating a company user. Leave company blank only for system-level users.</p>
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        {companyOptions.length ? <select value={form.companyId} onChange={(e) => updateField("companyId", e.target.value)} className="rounded-2xl border border-slate-200 px-4 py-3 text-sm"><option value="">System-level user / no company</option>{companyOptions.map((company) => <option key={company.companyId} value={company.companyId}>{company.name || company.companyId}</option>)}</select> : <input value={form.companyId} onChange={(e) => updateField("companyId", e.target.value)} placeholder="Company ID (optional)" className="rounded-2xl border border-slate-200 px-4 py-3 text-sm" />}
