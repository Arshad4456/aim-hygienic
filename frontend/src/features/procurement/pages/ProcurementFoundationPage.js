...
function canCreateGrn(row, goodsReceipts) { return ["approved", "partially_received"].includes(String(row?.status || "")) && !grnForPo(row, goodsReceipts); }
...
export default function ProcurementFoundationPage({ mode = "overview" }) {
  ...
  async function approvePO(id) { setSaving(true); setError(""); setNotice(""); try { await procurementService.approvePurchaseOrder(id); setNotice("Purchase order approved."); await load(); } catch (e) { setError(e.message || "Unable to approve purchase order"); } finally { setSaving(false); } }
  async function receivePO(id) { setSaving(true); setError(""); setNotice(""); try { const res = await procurementService.receivePurchaseOrder(id); setNotice(res?.message || "Draft GRN created. Post GRN to update stock and generate supplier invoice."); await load(); setTab("goods-receipts"); } catch (e) { setError(e.message || "Unable to receive purchase order"); } finally { setSaving(false); } }
  async function postGRN(id) { setSaving(true); setError(""); setNotice(""); try { await procurementService.postGoodsReceipt(id); setNotice("GRN posted. Inventory updated and supplier invoice generated."); await load(); setTab("supplier-finance"); } catch (e) { setError(e.message || "Unable to post GRN"); } finally { setSaving(false); } }
