...
function openInvoice(order) {
  ...
  const html = `<html><body style="font-family:Arial,sans-serif;padding:16px;color:#111;"><div style="display:flex;justify-content:space-between;align-items:center;"><div style="display:flex;align-items:center;gap:10px;"><div style="width:54px;height:54px;border-radius:10px;background:linear-gradient(135deg,#065f46,#10b981);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:22px;">RE</div><div><div style="font-weight:700;font-size:18px;">Rawyan ERP</div><div style="font-size:11px;color:#555;">Sales Invoice</div></div></div><div style="font-size:12px;text-align:right;"><div><b>Invoice #:</b> ${escapeHtml(order.orderNo || order.invoiceNo || order._id || '-')}</div><div><b>Date:</b> ${order.updatedAt ? new Date(order.updatedAt).toLocaleDateString() : '-'}</div></div></div><div style="margin-top:10px;font-size:12px;"><b>Invoice From:</b> ${escapeHtml(order.toWarehouseName || order.fromEntityName || 'Rawyan ERP')}</div><div style="font-size:12px;"><b>Bill To:</b> ${escapeHtml(order.distributorName || order.customerName || order.distributorId || '-')}</div><div style="font-size:12px;"><b>Territory:</b> ${escapeHtml(order.territoryName || order.areaName || '-')}</div><table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;width:100%;margin-top:12px;font-size:12px;"><thead><tr><th>#</th><th>Product</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${itemRows || '<tr><td colspan="5">No item details available</td></tr>'}</tbody></table><div style="margin-top:12px;display:flex;justify-content:flex-end;font-size:12px;"><div style="min-width:260px;"><div style="display:flex;justify-content:space-between;"><span>Total Amount:</span><strong>${Number(order.totalAmount || 0).toFixed(2)}</strong></div></div></div><div style="margin-top:16px;text-align:center;font-size:12px;">Thank you for business with Rawyan ERP.</div></body></html>`;
