...
export default function AddScreen() {
  ...
  const onSave = async () => {
    if (!form.fieldId || !form.name || !form.warehouseId || !form.regionId || !form.zoneId || !form.territoryId) {
      Alert.alert('Missing data', 'Please fill all required dropdowns and IDs.');
      return;
    }
    setSaving(true);
    try {
  ...
  return <ScrollView contentContainerStyle={styles.content}><Card><Text style={styles.title}>Add Field</Text>
    ...
    <Text style={styles.label}>Warehouse</Text><ScrollView horizontal contentContainerStyle={styles.wrap}>{warehouses.map((w) => <Text key={w._id} style={[styles.chip, form.warehouseId === w.warehouseId ? styles.active : null]} onPress={() => setField('warehouseId', w.warehouseId)}>{w.name}</Text>)}</ScrollView>
    ...
    <Text style={styles.label}>Territory</Text><ScrollView horizontal contentContainerStyle={styles.wrap}>{territories.filter((t) => !form.zoneId || t.zoneId === form.zoneId).map((t) => <Text key={t._id} style={[styles.chip, form.territoryId === t.areaId ? styles.active : null]} onPress={() => setField('territoryId', t.areaId)}>{t.name}</Text>)}</ScrollView>
