...
export default function FuelManagementScreen() {
  ...
  const [form, setForm] = useState({ vehicleId: '', tripType: 'company', tripDate: '', fromPlace: '', toPlace: '', startOdometer: '', endOdometer: '', liters: '', startMeterUrl: '', endMeterUrl: '', fuelReceiptUrl: '' });
  ...
  const saveTrip = async () => {
    ...
    try {
      await apiClient.post('/vehicle-management/trips', { ...form, liters: toNum(form.liters), startOdometer: toNum(form.startOdometer), endOdometer: toNum(form.endOdometer) });
      setForm({ vehicleId: '', tripType: 'company', tripDate: '', fromPlace: '', toPlace: '', startOdometer: '', endOdometer: '', liters: '', startMeterUrl: '', endMeterUrl: '', fuelReceiptUrl: '' });
      await load();
    ...
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <View style={styles.grid}>
        <Card>
          ...
          <TextInput style={styles.input} placeholder="Trip Date (YYYY-MM-DD)" value={form.tripDate} onChangeText={(v) => setForm((s) => ({ ...s, tripDate: v }))} />
          ...
          <TextInput style={styles.input} placeholder="Liters" value={form.liters} onChangeText={(v) => setForm((s) => ({ ...s, liters: v }))} keyboardType="numeric" />
          <TextInput style={styles.input} placeholder="Start Meter Proof URL" value={form.startMeterUrl} onChangeText={(v) => setForm((s) => ({ ...s, startMeterUrl: v }))} />
          <TextInput style={styles.input} placeholder="End Meter Proof URL" value={form.endMeterUrl} onChangeText={(v) => setForm((s) => ({ ...s, endMeterUrl: v }))} />
          <TextInput style={styles.input} placeholder="Fuel Receipt URL" value={form.fuelReceiptUrl} onChangeText={(v) => setForm((s) => ({ ...s, fuelReceiptUrl: v }))} />
        ...
        <Card>
          ...
          <TextInput style={styles.input} placeholder="Date (YYYY-MM-DD)" value={refuel.date} onChangeText={(v) => setRefuel((s) => ({ ...s, date: v }))} />
          ...
          <TextInput style={styles.input} placeholder="Receipt URL" value={refuel.receiptUrl} onChangeText={(v) => setRefuel((s) => ({ ...s, receiptUrl: v }))} />
      ...
      <Ledger title="Trip Ledger" total={filteredTrips.length} page={tripPageData.page} totalPages={tripPageData.totalPages} onFirst={() => setTripPage(1)} onPrev={() => setTripPage((p) => Math.max(1, p - 1))} onNext={() => setTripPage((p) => Math.min(tripPageData.totalPages, p + 1))} onEnd={() => setTripPage(tripPageData.totalPages)}>
        ...
        <ScrollView horizontal><View style={styles.tableWrap}><Row head cols={['Date', 'Vehicle / User', 'Trip Type', 'Route', 'Odometer', 'Distance', 'Liters', 'Start Proof', 'End Proof', 'Receipt']} />
          {tripPageData.rows.map((t) => { const v = map.get(t.vehicleId) || {}; return <Row key={t._id} cols={[String(t.tripDate || '').slice(0, 10), vehicleLabel(v), t.tripType || '-', `${t.fromPlace || '-'} → ${t.toPlace || '-'}`, `${t.startOdometer || '-'} / ${t.endOdometer || '-'}`, String(t.distance || '-'), String(t.liters || 0), t.startMeterUrl || '-', t.endMeterUrl || '-', t.fuelReceiptUrl || '-']} />; })}
...
function Filters({ vehicles, filters, setFilters, withTripType, withDates }) {
  return (
    <View>
      ...
      {withDates ? <View style={styles.row2}><TextInput style={[styles.input, styles.flex1]} placeholder="From (YYYY-MM-DD)" value={filters.from} onChangeText={(v) => setFilters((s) => ({ ...s, from: v }))} /><TextInput style={[styles.input, styles.flex1]} placeholder="To (YYYY-MM-DD)" value={filters.to} onChangeText={(v) => setFilters((s) => ({ ...s, to: v }))} /></View> : null}
    </View>
  );
}

function Ledger({ title, total, page, totalPages, onFirst, onPrev, onNext, onEnd, children }) {
  return <Card><Text style={styles.h2}>{title}</Text><Text style={styles.sub}>{total} entries · Page {page} of {totalPages}</Text>{children}<Pager page={page} totalPages={totalPages} onFirst={onFirst} onPrev={onPrev} onNext={onNext} onEnd={onEnd} /></Card>;
}
...
function Pager({ page, totalPages, onFirst, onPrev, onNext, onEnd }) {
  return <View style={styles.pager}><PagerBtn label="Start" onPress={onFirst} disabled={page <= 1} /><PagerBtn label="Previous" onPress={onPrev} disabled={page <= 1} /><PagerBtn label="Next" onPress={onNext} disabled={page >= totalPages} /><PagerBtn label="End" onPress={onEnd} disabled={page >= totalPages} /></View>;
}
...
const styles = StyleSheet.create({
  content: { padding: 12, gap: 12, paddingBottom: 30 }, title: { fontSize: 20, fontWeight: '700' }, subtitle: { marginTop: 4, color: '#6b7280' }, err: { marginTop: 6, color: '#b91c1c' },
  grid: { gap: 12 }, h2: { fontSize: 16, fontWeight: '700', marginBottom: 8 }, sub: { fontSize: 12, color: '#6b7280', marginBottom: 8 },
  ...
  row2: { flexDirection: 'row', gap: 8 }, flex1: { flex: 1 }, wrap: { flexDirection: 'row', gap: 8, marginBottom: 8, paddingVertical: 2 },
