...
export default function SecondaryOrdersScreen() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    setErr('');
    try {
      const res = await apiClient.get('/orders?limit=500');
      setOrders((res?.data?.orders || []).filter((row) => String(row.saleType || '').toLowerCase() === 'secondary'));
    } catch (error) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <Card>
        <Text style={styles.title}>Secondary Orders</Text>
          ...
          <View style={styles.tableWrap}>
            ...
            {requestRows.length === 0 ? <Text style={styles.help}>No secondary requests.</Text> : requestRows.map((row) => (
              <View key={row._id} style={[styles.dataRow, rowTone(row.status)]}>
                ...
                <Text style={styles.cell}>{row.unreadForAdmin ? 'Unread' : 'Read'}</Text>
                <View style={[styles.cell, styles.actionCell]}>
                  <ActionBtn label="Open" onPress={() => openRequest(row._id)} />
                  <ActionBtn label="Preview" onPress={() => openPreview(row)} />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <ScrollView contentContainerStyle={styles.modalContent}>
              ...
              <Field label="Customer Name" value={draft.customerName || '-'} />
