...
const dashboardLinks = [{ title: 'Dashboard', route: 'accountOfficer:dashboard' }];
...
export default function DashboardScreen({ navigation }) {
  ...
  const userName = user?.fullName || user?.name || 'Account Officer';
  const userRole = user?.role || 'Account Officer';
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 40, height: 40, borderRadius: 999, backgroundColor: '#ede9fe', alignItems: 'center', justifyContent: 'center' },
  avatarText: { color: '#6d28d9', fontWeight: '700' },
